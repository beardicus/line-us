...
class LineUs extends Nanobus {
  ...
  connect() {
    ...
    this._ws.onopen = async () => {
      ...
      this._ws.onmessage = (response) => {
        // resolve promises with responses as they come in
        this._responseQueue.shift()(response)
      }
    ...
    this._ws.onclose = () => {
      this.stop() // state will be "paused" after this
  ...
  async _send(cmd) {
    let responseEvent = new Promise((resolve, reject) => {
      this._responseQueue.push(resolve)
      // TODO: some sort of timeout-based rejection here?
    })
